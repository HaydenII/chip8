...
public:
	...
	bool OnUserUpdate(float fElapsedTime) override
	{
		...
		for (int x = 0; x < 32; x++) {
			...
			for (int y = 0; y < 64; y++) {
				if ((TempDisplay >> (63 - y)) & 0x0000000000000001) {
					Draw(y, x, olc::WHITE);
				}
				else {
					Draw(y, x, olc::BLACK);
...
int main() {
	...
	*/
	// Draw two characters
	//uint8_t program[] = { 0xA0, 0x19, 0x60, 0x00, 0x61, 0x00, 0xD0, 0x15, 0xA0, 0x05, 0x60, 0x09, 0x61, 0x0F, 0xD0, 0x15 };
	...
	std::vector<uint8_t> program = readFile("C:\\Users\\hayde\\Downloads\\IBM.ch8");
